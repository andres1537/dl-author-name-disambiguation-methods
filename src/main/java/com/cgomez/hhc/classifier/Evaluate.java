...
public class Evaluate {
    ...
    public class AuxCluster {
        ...
        public Collection<AuxCluster> getRealClusters(HashMap<Integer, AuxCluster> realGroups){
            HashMap<Integer, AuxCluster> aux = new HashMap<>();
            for (Citation c: citations.values()){
                AuxCluster g = aux.get(c.classId);
                if (g == null){
                    g = realGroups.get(c.classId);
                    aux.put(c.classId, g);
                    g.intersec = 0;
                }
                g.intersec++;
    ...
    private void initializeClusterMetrics(Citation i){
        ...
        Integer resultGroupId = i.predictedCluster;
        ...
        if (g == null){
            g = new AuxCluster(resultGroupId, ! false);
    ...
    public double computeKMetric(){
        ...
        for (AuxCluster rc: resultGroups.values()){
            for (AuxCluster tc: rc.getRealClusters(realGroups)){
                double intersection = Math.pow(tc.intersec,2.0);
                acp += intersection / (double) rc.size();
                aap += intersection / (double) tc.size();
            }
        }
            */
        ...
        for (AuxCluster rc: resultGroups.values()){
            for (AuxCluster tc: realGroups.values()){
                double intersection = Math.pow(tc.intersection(rc),2.0);
                acp += intersection / (double) rc.size();
                aap += intersection / (double) tc.size();
            }
        }

...
public class Cluster implements Comparable {
	...
	public void setCentroide(Base base){
		...
		Hashtable <String,Double> vetor = new Hashtable<String,Double>();
		
		for (Artigo a:this.getArticles()){
			
			a.setFeatureVector(base.getLstTerm(), true, 't', base.getLstTerm().size());
			
			/*if (a.getCoautores()!=null)
				for (String c:a.getCoautores())
					coautores.put(c, c);
			for (String t:a.getTitulo().split("[ ]"))
				termosTitulo.put(t, t);
			for (String v:a.getVeiculoPublicacao().split("[ ]"))
				termosVenue.put(v, v);
			*/
			// define um vetor de features com a soma dos pesos de todos os vetores
			FeatureVector fV = a.getFeatureVector(base.criaListaTermos(true), true, 't', base.getArtigos().size());
		...
		// cria um artigo com todos os dados de todos os artigos do cluster
		/*String []arrayCo=null;
		if (coautores.keySet().size()>0)
			arrayCo = new String[coautores.keySet().size()];
			int i=0;
			for (Object o: coautores.keySet()){
				arrayCo[i]=(String)o;
				i++;
			}
		String termosT="";
		for(String t:termosTitulo.keySet())
			termosT= termosT+t+" ";
		String termosV="";
		for(String v:termosVenue.keySet())
			termosV= termosV+v+" ";
		this.artigoCentroide = new Artigo (this.getNumber(), this.getNumber(), 0, this.getArticles().get(0).getAutor(), 
				arrayCo, termosT ,
				termosV);
		System.out.println("Centroide: "+this.artigoCentroide.toStringArqDen());
		StringBuffer geral = new StringBuffer();
		for (String c: coautores.keySet()){
			geral.append(c.replaceAll(" ", "_"));
			geral.append(" ");
		}
		for (String t: termosTitulo.keySet()){
			geral.append(t);
		...
		this.stringCentroide = geral.toString().substring(0, geral.length()-1);
		*/
	}

	public Artigo getArtigoCentroide(Base base) {
		if (artigoCentroide == null)
			this.setCentroide(base);
		return artigoCentroide;
	}

	public String getStringCentroide(Base base) {
		if (stringCentroide==null)
			this.setCentroide(base);
		return stringCentroide;
	}
	...
	public boolean similarAllAuthorNames(String nameAuthor, double limiar){
		...
		while (resultado && i <numArtigos) {
			if (!com.cgomez.hhc.hhc.Similarity.ComparacaoFragmentos(this.getArticles().get(i).getAutor(), nameAuthor, limiar))
				resultado = false;
			i++;
		}
		return resultado;

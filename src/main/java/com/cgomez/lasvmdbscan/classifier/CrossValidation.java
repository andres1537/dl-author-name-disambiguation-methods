...
public class CrossValidation {
    ...
    public void dataSetKIST(int numFolds) throws IOException, FileNotFoundException, InterruptedException{
        String set = "kisti";
    ...
    public void dataSetDBLP(int numFolds) throws IOException, FileNotFoundException, InterruptedException{
        String set = "dblp";
        //String[] groups = {"agupta","akumar","cchen","djohnson","jmartin","jrobinson","jsmith","ktanaka", "mbrown","mjones","mmiller","slee", "ychen", "jlee"};
        String[] groups = {"mjones","mmiller"};
    ...
    public void dataSetBDBComp(int numFolds) throws IOException, FileNotFoundException, InterruptedException{
        String set = "bdbcomp";
    ...
    private void execute(String[] groups, String dataset, String collection, int numFolds) throws IOException, FileNotFoundException, InterruptedException{
        ...
        for (String ambiguousGroup: groups){
            ...
            for (int i=0; i<numFolds; i++){
                ...
                if (empty){
                    tic = System.currentTimeMillis();
                    eval = new Evaluate(lasvmdbscan.oneClass(test));
                    testTime = System.currentTimeMillis() - tic;
                } else {
                    tic = System.currentTimeMillis();
                    eval = new Evaluate(lasvmdbscan.test(test));
                    testTime = System.currentTimeMillis() - tic;
                }

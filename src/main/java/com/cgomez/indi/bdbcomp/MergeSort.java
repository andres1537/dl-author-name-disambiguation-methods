...
public class MergeSort<E extends Comparable<E>>
{
  ...
  private void split(ArrayList<E> list, int start, int end)
  {
    if (end - start + 1 > 2)
    {
      ...
      split(list, start, middle);
      split(list, middle + 1, end);
      merge(list, start, middle, middle + 1, end);
    }
    else if (end - start + 1 == 2)
    {
      if (((Comparable)list.get(start)).compareTo((Comparable)list.get(end)) == 1) {
        swap(list, start, end);
      }
    }
    else {}
  ...
  private void merge(ArrayList<E> list, int start1, int end1, int start2, int end2)
  {
    ...
    ArrayList<E> listAux = new ArrayList();
    int p1 = start1;
    int p2 = start2;
    ...
    while (i < size)
    {
      E c1 = (Comparable)list.get(p1);
      E c2 = (Comparable)list.get(p2);
    ...
    if (exp1) {
      while (i < size)
      {
        listAux.add((Comparable)list.get(p2));
        p2++;
        i++;
      }
    } else if (exp2) {
      while (i < size)
      {
        listAux.add((Comparable)list.get(p1));
        p1++;
        i++;
      }
    }
    ...
    {
      E saux = (Comparable)listAux.get(h);
  ...
  {
    E aux1 = (Comparable)list.get(p1);
    E aux2 = (Comparable)list.get(p2);
    list.set(p1, aux2);
    list.set(p2, aux1);
  }
}

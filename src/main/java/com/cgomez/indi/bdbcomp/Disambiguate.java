...
public class Disambiguate
{
  ...
  static String database = "jdbc:mysql://localhost/bdbcomp2";
  ...
  public static void main(String[] args)
    throws Exception
  {
    ArrayList<Artigo> listArticles = createAuthorshipRecords();
    System.out.println("Printing the authorship records.");
    for (Artigo a : listArticles)
    {
      convert(a);
      System.out.println(a.toStringArqDen());
    }
    System.out.println("Ending of the printing.");
    
    Base base = new Base();
    base.getArtigos().addAll(listArticles);
    double simVenue = 0.3D;
    double simTitle = 0.3D;
    System.out.println("Starting disambiguation.");
    HHC.process(base, simTitle, simVenue);
    System.out.println("Ending of the disambiguation.");
    for (Artigo a : listArticles) {
      System.out.println(a.toStringArqDen() + "\t" + 
        a.getNumClasseRecebida());
    }
  }
  ...
  {
    String result = changeHTMLCodeToASC(string);
    
    Charset charset = Charset.forName("UTF-8");
    CharsetDecoder decoder = charset.newDecoder();
    CharsetEncoder encoder = charset.newEncoder();
    
    ByteBuffer bb = encoder.encode(CharBuffer.wrap(string));
  ...
  public static void convert(Artigo a)
    throws Exception
  {
    a.setAutor(changeHTMLCodeToASC(a.getAutor()));
    if (a.getCoautores() != null) {
      for (int i = 0; i < a.getCoautores().length; i++) {
        a.getCoautores()[i] = changeHTMLCodeToASC(a.getCoautores()[i]);
      }
    }
  }
  ...
  public static String stem(String str)
  {
    if (str != null)
    {
      ...
      for (int i = 0; i < strTemp.length; i++)
      {
        ...
        stemmer.stem();
        strResult = strResult + stemmer.toString() + " ";
      }
      return strResult;
    }
    return str;
